...
<xsl:output method="xml" encoding="UTF-8"/>
  ...
  <xsl:element name="datos_ventas">
    ...
    <xsl:element name="Total_T1">
      <xsl:value-of select="sum(ventas/datos/provincia/trimestre[@numero=1]/@cantidad)"></xsl:value-of>    
    </xsl:element>
    <xsl:element name="Total_T2">
      <xsl:value-of select="sum(ventas/datos/provincia/trimestre[@numero=2]/@cantidad)"></xsl:value-of>    
    </xsl:element>
    <xsl:element name="Total_T3">
      <xsl:value-of select="sum(ventas/datos/provincia/trimestre[@numero=3]/@cantidad)"></xsl:value-of>    
    </xsl:element>
       <xsl:element name="Total_T4">
      <xsl:value-of select="sum(ventas/datos/provincia/trimestre[@numero=4]/@cantidad)"></xsl:value-of>    
    </xsl:element>
  </xsl:element>
